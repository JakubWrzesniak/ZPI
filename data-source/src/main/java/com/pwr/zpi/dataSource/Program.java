...
import static com.pwr.zpi.dataSource.ShardManagementUtils.createOrGetShardMapManager;
import static com.pwr.zpi.dataSource.SqlDatabaseUtils.createDatabase;
...
public class Program {
    ...
    /**
     * The shard map manager, or null if it does not exist. It is recommended that you keep only one shard map manager instance in memory per
     * AppDomain so that the mapping cache is not duplicated.
     */
    ...
    private static void menuLoop() {
        ...
        shardMapManager = ShardManagementUtils.tryGetShardMapManager(Configuration.getShardMapManagerServerName(),
    ...
    private static void printRangeShardMapState() {
        ...
        if (!mappingsGroupedByShard.isEmpty()) {
            ...
            mappingsGroupedByShard.keySet().stream().sorted(Comparator.comparing(shard -> shard.getLocation().getDatabase())).forEach(shard -> {
    ...
    private static void printListShardMapState() {
        ...
        if (!mappingsGroupedByShard.isEmpty()) {
            // The shard map contains some shards, so for each shard (sorted by database name)
            // write out the mappings for that shard
            mappingsGroupedByShard.keySet().stream().sorted().forEach(shard -> {
    ...
    /**
     * Creates a shard map manager, creates a shard map, and creates a shard with a mapping for the full range of 32-bit integers.
     */
    public static void createShardMapManagerAndShard() {
        ...
        if (shardMapManager != null) {
            ConsoleUtils.writeWarning("Shard Map Manager %s already exists in memory", shardMapManagerServerName);
        }
        ...
        if (!StringUtilsLocal.isNullOrEmpty(shardMapManagerConnectionString)) {
            ...
            // Create shard map
            RangeShardMap<Integer> rangeShardMap = ShardManagementUtils.createOrGetRangeShardMap(shardMapManager,
                    Configuration.getRangeShardMapName(), ShardKeyType.Int32);

            ListShardMap<Integer> listShardMap = ShardManagementUtils.createOrGetListShardMap(shardMapManager, Configuration.getListShardMapName(),
                    ShardKeyType.Int32);
...
//            }
    ...
    public static void addShard() {
        int shardType = ConsoleUtils.readIntegerInput("1. Range Shard\r\n2. List Shard\r\n" + "Select the type of shard you want to create: ", 0,
    ...
    private static void dataDependentRouting() {
        ...
        ListShardMap<Integer> listShardMap = tryGetListShardMap();
        if (listShardMap != null) {
            DataDependentRoutingSample.executeDataDependentRoutingQuery(listShardMap, Configuration.getCredentialsConnectionString());
        }
    }
    ...
    private static void addRangeShard() {
        ...
        if (rangeShardMap != null) {
            ...
            int currentMaxHighKey = rangeShardMap.getMappings().stream().mapToInt(m -> (Integer) m.getValue().getHigh()).max().orElse(0);
            ...
            int newHighKey = ConsoleUtils.readIntegerInput(String.format("Enter the high key for the new range [default %1$s]: ", defaultNewHighKey),
    ...
    private static void addListShard() {
        ...
        if (listShardMap != null) {
            ...
            List<Integer> currentKeys = listShardMap.getMappings().stream().map(m -> (Integer) m.getValue()).sorted().collect(Collectors.toList());

            ArrayList<Integer> newKeys = new ArrayList<>();
            int newKey;
            ConsoleUtils.writeInfo("");
            do {
                newKey = ConsoleUtils.readIntegerInput("Enter the points to be mapped to the new shard." + " To stop press enter: ", 0,
                        input -> !currentKeys.contains(input));
                if (newKey > 0) {
                    newKeys.add(newKey);
                }
            }
    ...
    private static RangeShardMap<Integer> tryGetRangeShardMap() {
        ...
        RangeShardMap<Integer> rangeShardMap = shardMapManager.getRangeShardMap(Configuration.getRangeShardMapName(), ShardKeyType.Int32);
     ...
     */
    public static ListShardMap<Integer> tryGetListShardMap() {
        if (shardMapManager == null) {
            ConsoleUtils.writeWarning("Shard Map Manager has not yet been created");
            return null;
        }

        ListShardMap<Integer> listShardMap = shardMapManager.getListShardMap(Configuration.getListShardMapName(), ShardKeyType.Int32);
...
//    public String provisionNewTenant(String databaseName, Boolean isGlobal, int databaseKey) {
...
//        if (isGlobal) {
//            SqlDatabaseUtils.executeSqlScript(shardMapManagerServerName, databaseName, "global.sql");
//        } else {
...
//        tryGetListShardMap().createPointMapping(databaseKey, systangoShard);
//        return "TENANT_CREATED";
//    }

    public static void addNewOrganization(Integer id) {
        shardMapManager = ShardManagementUtils.tryGetShardMapManager(Configuration.getShardMapManagerServerName(),
                Configuration.getShardMapManagerDatabaseName());
        ListShardMap<Integer> listShardMap = tryGetListShardMap();
        if (listShardMap != null) {
            // Here we assume that the point start at 0, are contiguous, and are bounded
            List<Integer> currentKeys = listShardMap.getMappings().stream().map(m -> (Integer) m.getValue()).sorted().collect(Collectors.toList());
            ArrayList<Integer> newKeys = new ArrayList<>(List.of(id));
            if (newKeys.size() > 0) {
                ConsoleUtils.writeInfo("Creating shard for given list of points");
                CreateShardSample.createShard(listShardMap, newKeys);
            }
